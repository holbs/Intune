#Region: Detection
If (Test-Path -Path "$env:WINDIR\System32\Macromed\Flash") {Exit 0}
If (Test-Path -Path "$env:WINDIR\SysWOW64\Macromed\Flash") {Exit 0}
If (Test-Path -Path "$env:SYSTEMDRIVE\Users\*\AppData\Roaming\Adobe\Flash Player") {Exit 0}
If (Test-Path -Path "$env:SYSTEMDRIVE\Users\*\AppData\Roaming\Macromedia\Flash Player") {Exit 0}
If (Test-Path -Path 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\Adobe Flash*') {Exit 0}
If (Test-Path -Path 'HKLM:\SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\Adobe Flash*') {Exit 0}
#EndRegion
#Region: Uninstall
